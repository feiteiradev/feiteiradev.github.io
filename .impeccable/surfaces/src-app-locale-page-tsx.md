...
FIRST VIEWPORT (portrait-led split, Pedro's choice 2026-09-18): 12 columns. Columns 1–3, left of the axis: a tall colour portrait cropped hard to the grid, stretching the full height of the right column, no rounding or frame. Columns 4–12, hung from the axis: the name at poster scale, the role and place as one line under it, a single short statement (one line of promise, not the long paragraph), then the two doors "Está a recrutar?" (résumé icon) and "Precisa de algo construído?" (hammer icon) as ruled cells. Below, spanning all 12 columns: the work as a four-cell strip, Crudo, Feit-Y, Aircall, EngineAI in that order (most real first), three columns each; each cell leads with an icon (the apps' own App Store icons; an accent line icon for client work: headset for Aircall, braces for EngineAI) beside name, provenance and a dated status mark. No numerals: Pedro rejected them 2026-09-18. Phones: name, portrait, statement, doors, then the work cells stacked.
