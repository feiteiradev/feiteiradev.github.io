...
Blocks are separated by a 15% top hairline and 64px vertical padding (96px at lg). Rows inside them are 16-32px tall in padding, separated only by hairlines. The home hero is a portrait-led split at lg: the portrait spans columns 1-3 and stretches to the height of the column beside it (name, role line, one-line statement, doors in columns 4-12); the client work sits below the doors in the same two columns (4-12), icon beside name and provenance, with no status line. It fits the first viewport at 1280x800 and up. On phones the order is name, portrait (4:3), statement, doors, then the client cells.
